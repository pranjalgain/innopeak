...
/**
 * Same shape as `changePasswordSchema` minus `currentPassword` — there is nothing to prove for an
 * account that has never had one (Google/SSO-only). A factory for the same reason: built at
  ...
  return z
    .object({
      otp: z
        ...
        .length(OTP_LENGTH, t("errors.otpInvalid"))
        .regex(OTP_PATTERN, t("errors.otpInvalid")),
      newPassword: z
        .string()
...
export type SetPasswordSchema = ReturnType<typeof setPasswordSchema>;
export type SetPasswordFields = z.infer<SetPasswordSchema>;
