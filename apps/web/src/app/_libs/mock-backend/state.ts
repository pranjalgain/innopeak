...
/**
 * A handful more platform-admin-only tenants — same spirit as `DEMO_SECOND_TENANT_ID`'s own
 * comment ("purely so the Super Admin ... screens exercise their filters"), extended to the
 * Overview page's newer panels: Recent Businesses and the signup trend chart need more than two
 * data points spread over the last few months to read as a real, growing platform rather than an
 * empty chart, and Needs Attention needs at least one example of each of its three states
 * (disconnected / needs re-auth / never connected) to demonstrate its own priority ordering.
 * Deliberately not wired into any tenant-facing login flow — nobody signs in as these, they only
 * exist for the admin console to list.
 */
const OVERVIEW_DEMO_TENANTS: {
  id: string;
  name: string;
  ownerName: string;
  ownerEmail: string;
  ageDays: number;
  connection: "connected" | "needs_reauth" | "disconnected" | "never_connected";
}[] = [
  {
    id: "tenant_saltbox_grill",
    name: "Saltbox Grill",
    ownerName: "Lena Whitmore",
    ownerEmail: "lena@saltboxgrill.com",
    ageDays: 12,
    connection: "connected",
  },
  {
    id: "tenant_harbor_lights_inn",
    name: "Harbor Lights Inn",
    ownerName: "Derek Okafor",
    ownerEmail: "derek@harborlightsinn.com",
    ageDays: 38,
    connection: "needs_reauth",
  },
  {
    id: "tenant_pinecrest_dental",
    name: "Pinecrest Dental",
    ownerName: "Sofia Marchetti",
    ownerEmail: "sofia@pinecrestdental.com",
    ageDays: 65,
    connection: "disconnected",
  },
  {
    id: "tenant_blue_anchor_marina",
    name: "Blue Anchor Marina",
    ownerName: "Tomás Reyes",
    ownerEmail: "tomas@blueanchormarina.com",
    ageDays: 95,
    connection: "never_connected",
  },
];

function buildOverviewDemoTenants(): {
  tenants: MockTenant[];
  users: MockUser[];
  connections: MockConnection[];
} {
  ...
  OVERVIEW_DEMO_TENANTS.forEach((spec, index) => {
    // "disconnected" (has a location on record but its connection was revoked/removed) and
    // "connected"/"needs_reauth" (a live connection, healthy or not) both need a non-null
    // `activeLocationId` — `connectionStatusFor` reads "never connected" purely off that being
    // null, before it ever looks for a connection row.
    const activeLocationId = spec.connection === "never_connected" ? null : `location_${spec.id}`;
    tenants.push({
      id: spec.id,
      name: spec.name,
      status: "active",
    ...
    users.push({
      ...
      avatarUrl: null,
      locale: null,
      invitedAt: daysAgo(spec.ageDays),
      lastLoginAt: daysAgo(Math.max(0, spec.ageDays - index)),
    });
    if (spec.connection === "connected" || spec.connection === "needs_reauth") {
      connections.push({
        id: `connection_${spec.id}`,
        tenantId: spec.id,
        providerAccountId: `google-account-${spec.id}`,
        connectionStatus: spec.connection === "needs_reauth" ? "needs_reauth" : "active",
        connectedAt: daysAgo(spec.ageDays),
      });
    }
    // "disconnected" gets no connection row at all — `connectionStatusFor` reads a non-null
    // `activeLocationId` with no matching connection as "disconnected", the same shape a real
    // owner-revoked OAuth grant would leave behind.
  });
...
function buildInitialState(): MockDb {
  ...
  const overviewDemo = buildOverviewDemoTenants();
