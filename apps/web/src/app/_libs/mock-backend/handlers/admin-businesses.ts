...
/** Shared with `admin-overview.ts` — Recent Businesses and Needs Attention render the exact same
 *  row shape this list does, so this one mapping is the only place that shape is defined. */
export function toAdminBusinessDto(tenant: MockTenant) {
