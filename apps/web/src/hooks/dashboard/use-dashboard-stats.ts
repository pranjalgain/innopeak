...
interface UseDashboardStatsResult {
  ...
  escalationBreakdown: EscalationBreakdownRow[];
  isLoading: boolean;
  error: string | null;
}
...
export function useDashboardStats(range: DateRange): UseDashboardStatsResult {
  ...
  const [approvalBreakdown, setApprovalBreakdown] = React.useState<ApprovalBreakdownRow[]>([]);
  const [escalationBreakdown, setEscalationBreakdown] = React.useState<EscalationBreakdownRow[]>([]);
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      ...
      DashboardService.getApprovalBreakdown(range),
      DashboardService.getEscalationBreakdown(range),
    ])
      .then(([statsResult, ratingResult, attentionResult, approvalResult, escalationResult]) => {
        if (cancelled) return;
        setStats(statsResult);
        setRatingDistribution(ratingResult);
        setAttentionReviews(attentionResult);
        setApprovalBreakdown(approvalResult);
        setEscalationBreakdown(escalationResult);
      })
      .catch(() => {
